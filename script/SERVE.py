...
def create_BAM_index(args, bam):
    cmd = 'samtools index'+' -@ '+str(args.nthread) \
                          +' '+bam

    return cmd

def Extract(args):
    cmd = 'samtools view'+' -@ '+str(args.nthread) \
                         +' -L '+args.erv_bed \
                         +' -bS' \
                         +' -o '+ERV_bam \
                         +' '+align_bam
...
ERV_bam = align_dir+'/'+args.prefix+'_TE.bam'
...
ERV_gtf = qc_dir+'/'+args.prefix+'_SERVE.gtf'
...
#	subprocess.check_call(cmd, shell=True, executable='/bin/bash')
	...
	cmd = create_BAM_index(args, align_bam)
	subprocess.check_call(cmd, shell=True, executable='/bin/bash')

	cmd = Extract(args)
	subprocess.check_call(cmd, shell=True, executable='/bin/bash')

	cmd = create_BAM_index(args, ERV_bam)
	subprocess.check_call(cmd, shell=True, executable='/bin/bash')
	...
	shutil.copyfile('./2_assem/trinity_ERV/Trinity-GG.fasta.gene_trans_map',ERV_gene_map)
...
#	subprocess.check_call(cmd, shell=True, executable='/bin/bash')
	subprocess.check_call('rm -rf ./2_assem/trinity_ERV', shell=True, executable='/bin/bash')
